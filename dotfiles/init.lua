...
vim.api.nvim_create_autocmd({"BufEnter", "FileType"}, {
  callback = function() 
    ...
    if vim.bo.filetype == "rust" then 
      vim.b._reg_dir = vim.fn.system("find $(echo ${CARGO_HOME:-$HOME/.cargo})/registry/src  -maxdepth 1 -mindepth 1 -print -quit")
    else 
      vim.b._reg_dir = vim.fn.system("python3 -c 'import site; print(site.getsitepackages()[0])'"):gsub("%s+$", "") end
    if vim.bo.filetype == "netrw" then
      vim.b._search_path, excs = vim.b.netrw_curdir, { ".git", "*.egg-info", "__pycache__", "wandb", "target" }
    else
      vim.b._search_path, excs = vim.fn.getcwd(), { ".git", "*.egg-info", "__pycache__", "wandb", "target", ".venv" } end
...
vim.keymap.set("n", "<leader>g", function() vim.ui.input({ prompt = "> " }, function(p) if p then 
  if ext(string.format("grep -IEnr %s '%s' %s", vim.b._g_excs, p, vim.b._search_path), true) then vim.cmd("cgetbuffer | bd | copen | cc") end 
end end) end)
vim.keymap.set("n", "<leader>f", function() vim.ui.input({ prompt = "> " }, function(p) if p then 
  ...
  if #l == 1 then vim.cmd("edit " .. vim.fn.fnameescape(l[1])) end
end end) end)
