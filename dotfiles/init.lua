...
vim.api.nvim_create_autocmd("FileType",     { callback = function() local i = 4 for _, line in ipairs(vim.api.nvim_buf_get_lines(0, 0, 50, false)) do local c = line:match("^(%s+)%S") if c then i = math.min(i, #c) end end 
  vim.opt_local.shiftwidth=i vim.opt_local.tabstop=i vim.opt_local.softtabstop = i end , })
...
local function qf(cls) local items, bufnr = {}, vim.api.nvim_get_current_buf() 
...
local function extc(cmd, qf, cls, novs) o = vim.fn.systemlist(cmd) if o and #o > 0 then vim.cmd(novs and "enew" or "vnew") vim.api.nvim_buf_set_lines( 0, 0, -1, false, o) scratch() if qf then qf(cls) end end end
