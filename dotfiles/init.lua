...
local function ext(c, novs, autoopen, qf)
    ...
    if autoopen and #o == 1 then vim.cmd("edit " .. vim.fn.fnameescape(o[1])) end
...
vim.keymap.set("n", "<space>g", function() vim.ui.input({}, function(p) if p then 
    ext(string.format("grep -IEnr %s '%s' %s", vim.b._g_excs, p, vim.b._sp), true, false, true) end end) end)
vim.keymap.set("n", "<space>f", function() vim.ui.input({}, function(p) if p then 
    ext(string.format("find %s %s -path '*%s*' -print ", vim.b._sp, vim.b._f_excs, p), true, true) end end) end)
