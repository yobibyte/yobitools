...
vim.api.nvim_create_autocmd({"BufEnter", "FileType"}, { callback = function() t(4)
    ...
    vim.b._sp = vim.fn.shellescape(vim.bo.filetype == "netrw" and vim.b.netrw_curdir or vim.fn.getcwd())
...
vim.keymap.set("n", "<space>g", function() vim.ui.input({}, function(p) if p then 
    ext(string.format("grep -IEnr %s '%s' %s", vim.b._g_ex, p, vim.b._sp), true, true) end end) end)
vim.keymap.set("n", "<space>f", function() vim.ui.input({}, function(p) if p then 
    ext(string.format("find %s %s -path '*%s*' -print ", vim.b._sp, vim.b._f_ex, p), true) end end) end)
