...
local function scratch_to_quickfix()
  ...
  end end end end vim.api.nvim_buf_delete(bufnr, { force = true }) vim.fn.setqflist(items, "r") vim.cmd("copen | cc") end
local function extcmd(cmd, quickfix) 
  output = vim.fn.systemlist(cmd) if not output or #output == 0 then return end
  vim.cmd("vnew") vim.api.nvim_buf_set_lines( 0, 0, -1, false, output) scratch() if quickfix then scratch_to_quickfix() end end
