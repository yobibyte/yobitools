...
vim.keymap.set("n", "<leader>l", function() local ft = vim.bo.filetype if ft == "rust" then extc("cargo check && cargo clippy") elseif ft == "python" then extc("ruff check --output-format=concise --quiet " .. vim.fn.expand("%"), true) vim.cmd("edit") end end)
