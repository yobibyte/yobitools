...
require('lazy').setup({'yobibyte/vim-fugitive', 'yobibyte/vim-sleuth', 'yobibyte/undotree', 'yobibyte/Comment.nvim',
  ...
  {'yobibyte/nvim-treesitter', 
    ...
    opts = {ensure_installed = { 'c', 'cpp', 'python', 'rust', 'bash', 'zig' }, auto_install = true, sync_install = false, highlight = { enable = false }, indent = { enable = true },
...
vim.api.nvim_create_user_command("SearchDocs", function() builtin.grep_string({search = '',cwd = vim.fn.expand('%:p:h'),}) end, {})
vim.api.nvim_create_user_command("SearchDocFiles", function() builtin.find_files({cwd = vim.fn.expand('%:p:h'),}) end, {})
